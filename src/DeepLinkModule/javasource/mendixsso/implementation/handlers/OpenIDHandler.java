...
import mendixsso.proxies.MendixSSOSettings;
...
public class OpenIDHandler extends RequestHandler {
    ...
    private void login(IMxRuntimeRequest req, IMxRuntimeResponse resp, IContext context) throws Exception {
        ...
        if (session != null && !session.getUser(context).isAnonymous()) {
            ...
            final MendixSSOSettings mendixSSOSettings = Microflows.getOrCreateMendixSSOSettings(context);
            final Prompt.Type promptType;
            if (mendixSSOSettings.getSilentAuthentication() && promptValue.equals(Prompt.Type.NONE.toString())) {
    ...
    private void performAuthorizeRequest(IMxRuntimeRequest req, IMxRuntimeResponse resp, IContext context, String continuation, final Prompt.Type promptType) throws Exception {
        ...
        final MendixSSOSettings mendixSSOSettings = Microflows.getOrCreateMendixSSOSettings(context);
        // Specify scope
        String defaultScopes = Constants.getOpenIdConnectScopes();
        final Optional<String> additionalScopes = Optional.ofNullable(mendixSSOSettings.getAdditionalScopes(context));
        defaultScopes = StringUtils.trim(defaultScopes + " " + additionalScopes.orElse(""));

        final Scope scope = Scope.parse(defaultScopes);
        final IdentityProviderMetaData discovered = IdentityProviderMetaDataCache.getInstance().getIdentityProviderMetaData();
        // Compose the request
